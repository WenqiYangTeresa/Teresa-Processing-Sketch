...
void draw()
{
  ...
  for (i = 0; i<sines.length; i++) // go through all the sines; length is 20
  {
    ...
    if (trace) {
      ...
      stroke(80, 0, 255*(float(i)/sines.length), alpha); // blue
      fill(120, 0, 50, alpha/2); // also, um, blue
    ...
    if (trace) rect(0, radius, erad*6, 1); // draw with erad if tracing; the size and position of each dust
    popMatrix(); // go down one level
    
    pushMatrix(); // go up one level
    translate(0, radius); 
    rectMode(CENTER);// move to sine edge
    if (!trace) rect(0,0,erad,5); //ellipse(0, 0, 5, 5); // draw a little circle
    popMatrix(); // go down one level
