...
//when you click and hold on the lower righr corner, you will find a little red square on your mouse
//patterns composed by pixels will be genertate and it will swipe over the canvas diagonally
//move and hold the mouse around the area of the lower right, 
//you will get different effects.
//PS.I dont quite understand this. I mess it up a lot...
...
void draw()
{
  ...
  for (i=0; i<h; i++) // these pixels are the rows
  {
    for (j=0; j<w; j++) // thise pixels are the columns
    {
      ...
      {
        gx+=red(p[k])*gx_kernel[k]*x_amp*2; 
        gy+=red(p[k])*gy_kernel[k]*y_amp*2;
      }
