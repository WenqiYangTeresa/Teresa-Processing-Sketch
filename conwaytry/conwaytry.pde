...
void draw()
{
  ...
  for (i=0; i<h; i++) // these pixels are the rows
  {
    for (j=0; j<w; j++) // thise pixels are the columns
    {
      ...
      {
        c[1-which].pixels[i*w + j] = color(235, 140, 115);
      } else if (center==0 && sum==3) // dead... become alive
      {
        c[1-which].pixels[i*w + j] = color(125, 200, 205); //green of alive>28 to show
      } else // die (or stay dead)
      {   
        c[1-which].pixels[i*w + j] = color(60, 0, 150); //the green of dead<28 to hide
      }
    }
  }
